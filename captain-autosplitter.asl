...
state("nestopia")
{
    byte city:          "nestopia.exe", 0x1B2BCC, 0x00, 0x08, 0x0C, 0x0C, 0xB5;
    byte complete:      "nestopia.exe", 0x1B2BCC, 0x00, 0x08, 0x0C, 0x0C, 0x43C;
    byte redskull:      "nestopia.exe", 0x1B2BCC, 0x00, 0x08, 0x0C, 0x0C, 0x50D;
    byte start:         "nestopia.exe", 0x1B2BCC, 0x00, 0x08, 0x0C, 0x0C, 0x144;
}

state("mednafen")
{
    byte city:          "mednafen.exe", 0xBE1D2D;   
    byte complete:      "mednafen.exe", 0xBE20B4;
    byte redskull:      "mednafen.exe", 0xBE2185;
    byte start:         "mednafen.exe", 0xBE1DBC;
}
...
split
{
    ...
    if (settings["bRedSkull"] && current.city == 25 && old.redskull == 150 && current.redskull == 255) return true;
    if (settings["bWizard"] && current.city == 26 && old.complete == 3 && current.complete == 6) return true;
    if (settings["bWizard"] && current.city == 43 && old.complete == 3 && current.complete == 6) return true;
